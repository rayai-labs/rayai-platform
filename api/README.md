...
This project uses **sqlc** to generate type-safe Pydantic models and database queries from SQL migrations.
...
This generates:
- **Python types** (`api/app/database/generated/`) - Pydantic models + async/sync query methods
- **TypeScript types** (`app/src/lib/database.types.ts`) - Frontend database types

### Individual Targets

```bash
make generate-python      # Generate Python types only (sqlc)
make generate-typescript  # Generate TypeScript types only
make install-sqlc         # Install sqlc tool
```
...
**Python (sqlc):**
```python
from app.database.generated.querier_async import AsyncQuerier
from app.database.generated.models import SandboxStatus

querier = AsyncQuerier(db_conn)
sandbox = await querier.create_sandbox(user_id=user_id, status=SandboxStatus.ACTIVE)
sandboxes = await querier.get_sandboxes_by_user(user_id=user_id)
```
...
type Sandbox = Database['public']['Tables']['sandbox']['Row']
```

### Adding New Queries

1. Add SQL queries to `supabase/queries/*.sql`:
```sql
-- name: GetActiveSandboxes :many
SELECT * FROM sandbox WHERE status = 'active';
```
...
```python
active = await querier.get_active_sandboxes()
```
...
│   │       ├── querier_async.py  # Async query methods
│   │       └── querier_sync.py   # Sync query methods
│   ├── routes/
...
├── scripts/
│   └── db_reset.sh        # Database reset script
